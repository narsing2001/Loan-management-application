...
public class UserRolesController {
    ...
    // 1. Fetch all UserRole mappings for a given user id---------------------------------------------------------------
    @Operation(
            summary = "Get roles by user ID",
            description = "Fetches all roles assigned to the specified user."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<ApiResponse<List<UserRoleDTO>>> getUserRolesByUserId(
    ...
    // 2. Fetch all UserRole mappings for a given role------------------------------------------------------------------
    @Operation(
            summary = "Get users by role ID",
            description = "Fetches all users assigned to the specified role."
    )
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Role mappings fetched successfully",
                    content = @Content(schema = @Schema(implementation = ApiResponse.class)))
    })

    //2. Fetch all UserRole mappings for a given role-------------------------------------------------------------------
    @GetMapping("/role/{roleId}")
    public ResponseEntity<ApiResponse<List<UserRoleDTO>>> getUserRolesByRoleId(
    ...
    //3.assign a role to the particular user if the role is not exist---------------------------------------------------
    @PostMapping("/assign")
    public ResponseEntity<ApiResponse<UserRoleDTO>> assignRoleToUser(

            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Request body containing userId, roleId, and assignedBy",
                    required = true,
                    content = @Content(schema = @Schema(implementation = AssignRoleDTO.class))
            )
            @RequestBody AssignRoleDTO request) {
    ...
    @Operation(
            ...
            description = "Removes the specified role from the given user."
    )
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Role removed successfully",
                    content = @Content(schema = @Schema(implementation = ApiResponse.class)))
    })
    //4.remove the particular role from the user using the roleId--------------------------------------------------------
